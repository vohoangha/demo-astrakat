...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onClose }) => {
  ...
  // NEW: Added web_access to state with empty default
  const [newUser, setNewUser] = useState<{ username: string; password: string; credits: number | string; role: string; team: string; web_access: WebAccessType }>({ 
      username: '', password: '', credits: '', role: '', team: '', web_access: '' 
  });
  const [isTeamDropdownOpen, setIsTeamDropdownOpen] = useState(false);
  // NEW: Added web_access to bulk state with empty default
  ...
  const handleCreateUser = (e: React.FormEvent) => {
    ...
    if (isBulkRegister) {
        ...
        for (const u of validUsers) {
            ...
            if (!u.web_access) {
                showNotification(`Portal Access required for '${u.username}'`, "warning");
                return;
            }
        }
        ...
        executeSecureAction(`Register ${validUsers.length} new users?`, async (password) => {
             ...
             if (result.success) {
                ...
                // Reset bulk form
                setBulkUsers([{ username: '', password: '', credits: '', role: '', team: '', web_access: '' }]); 
                fetchUsers();
            } else { 
        ...
        if (!newUser.web_access) {
            showNotification("Please select Portal Access", "warning");
            return;
        }
        ...
        executeSecureAction(`Initialize User: ${finalUsername}?`, async (password) => {
            ...
            setNewUser({ username: '', password: '', credits: '', role: '', team: '', web_access: '' }); 
            fetchUsers();
        });
    }
  };
  ...
  const addBulkRow = () => { if (bulkUsers.length >= 10) return; setBulkUsers(prev => [...prev, { username: '', password: '', credits: '', role: '', team: '', web_access: '' }]); };
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 overflow-hidden font-sans">
      ...
      <div className="relative w-full max-w-7xl h-[90vh] flex flex-col animate-in zoom-in-95 slide-in-from-bottom-4 duration-500 shadow-2xl rounded-3xl overflow-hidden border border-[#e2b36e]/20 ring-1 ring-[#e2b36e]/10 bg-[#103742]/60 backdrop-blur-2xl">
         ...
         <div className="flex-1 relative min-w-0 bg-gradient-to-b from-transparent to-[#09232b]/50 overflow-hidden">
            ...
            {/* ... [Tabs: Injection, Members, Transactions same as before] ... */}
            {activeTab === 'injection' && (
                        ...
                        {!isBulkRegister ? (
                            <GlassCard className="p-8 md:p-10 border-white/10 relative">
                                ...
                                <form onSubmit={handleCreateUser} className="space-y-5">
                                    ...
                                    {/* UPDATED WEB ACCESS SELECTION */}
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-bold text-[#e2b36e]/40 uppercase tracking-widest pl-1">Portal Access</label>
                                        <div className="relative">
                                            <select 
                                                value={newUser.web_access} 
                                                onChange={(e) => setNewUser({...newUser, web_access: e.target.value as any})} 
                                                className={`w-full bg-black/20 border border-white/10 rounded-xl py-3.5 px-4 appearance-none focus:outline-none focus:border-[#e2b36e]/50 cursor-pointer ${newUser.web_access ? 'text-[#e2b36e]' : 'text-[#e2b36e]/40'}`}
                                            >
                                                <option value="" disabled className="bg-[#09232b] text-[#e2b36e]/50">Select Portal</option>
                                                <option value="ALL" className="bg-[#09232b] text-[#e2b36e]">ALL</option>
                                                <option value="EK" className="bg-[#09232b] text-[#e2b36e]">EK Astra Only</option>
                                                <option value="KAT" className="bg-[#09232b] text-[#e2b36e]">Astra KAT Only</option>
                                            </select>
                                            <Globe className="absolute right-4 top-1/2 -translate-y-1/2 text-[#e2b36e]/30 pointer-events-none" size={16} />
                                        </div>
                                    </div>
                                ...
                                <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-2">
                                    {bulkUsers.map((u, idx) => (
                                        <div key={idx} className="grid grid-cols-12 gap-2 animate-in fade-in slide-in-from-left-2 duration-300">
                                            ...
                                            <div className="col-span-2"><select value={u.team} onChange={(e) => updateBulkUser(idx, 'team', e.target.value)} className={`w-full bg-white/5 border border-white/10 rounded-lg px-1 py-2 text-[10px] focus:outline-none ${u.team ? 'text-[#e2b36e]' : 'text-[#e2b36e]/40'}`}><option value="" disabled className="bg-[#09232b]">Team</option>{TEAMS.map(t => <option key={t} value={t} className="bg-[#09232b]">{t}</option>)}</select></div>
                                            {/* UPDATED BULK WEB ACCESS */}
                                            <div className="col-span-3">
                                                <select 
                                                    value={u.web_access} 
                                                    onChange={(e) => updateBulkUser(idx, 'web_access', e.target.value)} 
                                                    className={`w-full bg-white/5 border border-white/10 rounded-lg px-1 py-2 text-[10px] focus:outline-none ${u.web_access ? 'text-[#e2b36e]' : 'text-[#e2b36e]/40'}`}
                                                >
                                                    <option value="" disabled className="bg-[#09232b] text-[#e2b36e]/50">Portal</option>
                                                    <option value="ALL" className="bg-[#09232b]">ALL</option>
                                                    <option value="EK" className="bg-[#09232b]">EK</option>
                                                    <option value="KAT" className="bg-[#09232b]">KAT</option>
                                                </select>
                                            </div>
